...
import {getLeagueTeams, getTeams} from "../api/index";
import noData from "../assets/icons/NoData.png";



function SearchEquipos({match}) {
    ...
    const fetchData = async () => {
        if(match.params.searchCriteria === 'All'){
            const response  = await getTeams(match.params.searchCriteria);
            setResult(response.data.recordset);
        }
        else{
            const response  = await getLeagueTeams(match.params.searchCriteria);
            setResult(response.data.recordset);
        }
    };
